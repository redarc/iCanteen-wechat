 ...
 */
var OAuth = function (corpid, corpsecret, getToken, saveToken) {
  this.corpid = corpid;
...
OAuth.prototype._getUser = function (userId, accessToken, callback) {
  var url = 'https://qyapi.weixin.qq.com/cgi-bin/user/get';
  var info = {
 ...
 * @param {Function} callback 回调函数
 */
OAuth.prototype.getUser = function (userId, callback) {
  ...
  this.getToken(openid, function (err, data) {
    ...
    if (token.isValid()) {
      that._getUser(userId, token.data.access_token, callback);
    } else {
...
OAuth.prototype.getUserByCode = function (code, callback) {
  ...
  this.getAccessToken(code, function (err, result) {
    ...
    that.getUserId(access_token, code, corpid, function(err, result){
      console.log('------ userid ---');
      console.log(result)
      console.log('------ userid ---');

      that._getUser(result.UserId, access_token ,callback);
    });
  });
};
 ...
 */
OAuth.prototype.getUserId = function(accessToken, code, corpid, callback) {
